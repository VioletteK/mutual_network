...
# for i in [13,14]:
    ...
    for time_delay in Time_delay:
        ...
        #
        [U,V]=np.gradient(Recorded_cell[injection_start+time_delay])
    ...
    fig = plt.figure()
    plt.imshow([[weighted_mean([Norm_vect[int((injection_start+time_delay)/dt)][i][j] for time_delay in Time_delay], np.linspace(0,10,len(Time_delay))) for j in range(window)] for i in range(window)], interpolation = 'none',vmin = 0,vmax=0.015, cmap = 'viridis')
    plt.colorbar()
