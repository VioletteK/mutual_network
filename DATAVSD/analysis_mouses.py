...
for folder in ['Mouses_3-5-6/20160912/','Mouses_3-5-6/20160914/','Mouses_3-5-6/20160916/']:
    for i in range(1,21):
        ...
        plt.close()
        fig.clf()
        print("The max value of the mean VM is "+str(max(VM_moyen)))

        if max(VM_moyen)>=0.0006:
            ...
            for time_delay in Time_delay:
                ...
                plt.imshow(VM[injection_start+time_delay], cmap = RdBu,interpolation='none', vmin =0 , vmax =0.006 )
                plt.colorbar()
                fig.add_subplot(2,1,2)
                plt.imshow(Recorded_cell_brut[injection_start+time_delay], cmap = 'inferno',interpolation='none', vmin = 0.025, vmax = 0.1)
                #plt.clim([0,0.07])
                plt.colorbar()
